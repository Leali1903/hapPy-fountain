...
def button_mood(action):  # ABSTRAKTER GESTALTEN? zB (msg,x,y,w,h,ic,ac,action)
    ...
    for event in pygame.event.get():
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if (SCREEN_HEIGHT/2) > mouse[1] > ((SCREEN_HEIGHT/2) - height_moodimage_y) and (SCREEN_WIDTH/2) > mouse[0] > ((SCREEN_WIDTH/2) - width_moodimage_x):
                click_output = 'happy'
                action(click_output)
            elif (SCREEN_HEIGHT/2) > mouse[1] > ((SCREEN_HEIGHT/2) - height_moodimage_y) and ((SCREEN_WIDTH/2) + width_moodimage_x) > mouse[0] > (SCREEN_WIDTH/2):
                click_output = 'sad'
                action(click_output)
            elif ((SCREEN_HEIGHT/2) + height_moodimage_y) > mouse[1] > (SCREEN_HEIGHT/2) and (SCREEN_WIDTH/2) > mouse[0] > ((SCREEN_WIDTH/2) - width_moodimage_x):
                click_output = 'party'
                action(click_output)
            elif ((SCREEN_HEIGHT/2) + height_moodimage_y) > mouse[1] > (SCREEN_HEIGHT/2) and ((SCREEN_WIDTH/2) + width_moodimage_x) > mouse[0] > (SCREEN_WIDTH/2):
                click_output = 'chillen'
                action(click_output)
    return click_output
...
def welcomeloop():
    ...
    while not welcomeexit:
        ...
        screen.blit(hf_logo, ((SCREEN_WIDTH / 2 - (190/2)), ((SCREEN_HEIGHT / 2) - 230)))  # Option: Logo statt Text
        mousecursor()
...
hf_logo = pygame.transform.scale(hf_logo, (190, 120))

# interaktiver Klick-Button 'next'
BUTTON_WIDTH = 200
BUTTON_HEIGHT = 100
...
width_moodimage_x = 300
height_moodimage_y = 300
